...
Original file is located at
    https://colab.research.google.com/drive/1f58_YCiliFJnGZfNtKjn2pcXwcaxfH_O
"""
...
#                            title="Median Salary by Job Title", color='salary_usd', color_continuous_scale='Viridis',range_x=[90000,110000])
...
subprocess.run(['pip', 'install', 'streamlit', 'pyngrok'], check=True)

print('Streamlit and pyngrok installed successfully!')

# Terminate the Streamlit process
print("Attempting to stop Streamlit and ngrok processes...")

global streamlit_process # Declare global here to access it if it exists
if 'streamlit_process' in globals() and isinstance(streamlit_process, subprocess.Popen) and streamlit_process.poll() is None:
    streamlit_process.kill() # Use kill for a more forceful termination
    print("Streamlit process forcefully killed.")
elif 'streamlit_process' in globals() and isinstance(streamlit_process, subprocess.Popen):
    print("Streamlit process was already stopped or exited.")
else:
    print("streamlit_process variable not found or not a Popen object in global scope.")

# Terminate any ngrok tunnels
try:
    ngrok.kill()
...
print("Termination attempt completed.")
...
# IMPORTANT: Go to '🔑' (Secrets) on the left sidebar, add a new secret named 'NGROK_AUTH_TOKEN'
# and paste your ngrok auth token as its value.
ngrok_auth_token = userdata.get('NGROK_AUTH_TOKEN')
if ngrok_auth_token:
    ngrok.set_auth_token(ngrok_auth_token)
    print("ngrok authentication token set.")
else:
    print("WARNING: NGROK_AUTH_TOKEN not found in Colab Secrets. Ngrok may fail to connect.")
    print("Please add your ngrok auth token to Colab Secrets (named NGROK_AUTH_TOKEN) for successful connection.")

# Make streamlit_process global so it can be accessed by other cells for termination
...
time.sleep(5)  # Wait for 5 seconds to ensure Streamlit starts
...
try:
    ...
    print("Please ensure Streamlit is running and your ngrok auth token is set in Colab Secrets.")
    print("You can try checking Streamlit logs for more details.")


# Optional: Information on how to display Streamlit output logs (useful for debugging)
print("--- Streamlit Output Logs (if needed for debugging) ---")
print(f"To view Streamlit's stdout (if process is still active), run: `!tail -n 20 /proc/{streamlit_process.pid}/fd/1`")
print(f"To view Streamlit's stderr (if process is still active), run: `!tail -n 20 /proc/{streamlit_process.pid}/fd/2`")
print("--------------------------------------------------")

# Terminate the Streamlit process and ngrok tunnels
print("Attempting to stop Streamlit and ngrok processes...")
...
if 'streamlit_process' in globals() and isinstance(streamlit_process, subprocess.Popen) and streamlit_process.poll() is None:
    ...
    print("streamlit_process variable not found or not a Popen object in global scope. Cannot kill Streamlit process.")
...
try:
    ...
    print(f"Error killing ngrok tunnels: {e}. (This might happen if no tunnels were active)")
